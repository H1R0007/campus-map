...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      // перезагружает страницу сама — в том числе посреди маршрута. Service worker
      // регистрирует само приложение (`src/pwa/registerServiceWorker.ts`): модулю
      // регистрации плагина нужен `workbox-window` (запись 27).
      registerType: 'prompt',
      ...
      workbox: {
        // Первая установка сразу берёт открытую страницу под управление: иначе
        // данные и планы первого визита не попали бы в кэш, и без связи навигатор
        // не открылся бы до второго визита. Обновление при этом ждёт согласия:
        // `skipWaiting` не включён (запись 27).
        clientsClaim: true,
