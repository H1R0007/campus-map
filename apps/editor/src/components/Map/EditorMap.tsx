...
const MapEventHandler: React.FC = () => {
  ...
  // safety: если mouseup произошЄл вне карты Ч вернуть dragging
  useEffect(() => {
  ...
  useMapEvents({
    click: (e) => {
      ...
      if (activeTool === 'select' && !dom.shiftKey) {
        // клик по пустому месту Ч сброс выделени€
        clearSelection();
      }
    },

    // Ctrl+ѕ ћ selection box
    mousedown: (e) => {
      const dom = e.originalEvent as MouseEvent;
  ...
  // на случай Esc Ч отмена selection box
  useEffect(() => {
