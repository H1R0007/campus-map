 ...
 *
 * Связность здесь намеренно не проверяется: части открытого плана считает
 * `planStats` для обзора плана, связность всего графа —
 * `findConnectedComponents` ядра. Третья реализация обхода дала бы третий
